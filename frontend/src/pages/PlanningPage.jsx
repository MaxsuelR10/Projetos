...
export function PlanningPage() {
  ...
  async function saveGoal(event) {
    event.preventDefault()
    const form = new FormData(event.target)
    try { await planningService.saveGoal({ name: form.get('name'), targetAmount: parseCurrency(goalAmounts.targetAmount), currentAmount: parseCurrency(goalAmounts.currentAmount) || '0', deadline: form.get('deadline') || null }); event.target.reset(); setGoalAmounts({ targetAmount: '', currentAmount: '' }); await load() } catch (requestError) { setError(getApiError(requestError)) }
  }
