...
function WishCard({ item, currency, onEdit, onPurchase, onRestore, onDelete, busy }) {
  ...
  return <article className={`wish-card ${purchased ? 'is-complete' : ''}`}>
    ...
    <ActionMenu label={`Ações para ${item.name}`} showPrimary={false} items={[
      { label: 'Editar', onSelect: onEdit, disabled: busy },
...
function ReminderCard({ item, currency, onEdit, onDone, onRestore, onDelete, busy }) {
...
export function WishesPage() {
  ...
  function closeDrawer() { setDrawer(''); setEditingWish(null); setEditingReminder(null); setWishForm(emptyWish); setReminderForm(emptyReminder); setError('') }
  function openWishForm(item = null) {
    setEditingWish(item)
    setEditingReminder(null)
    setWishForm(item ? { name: item.name, amount: item.amount, url: item.url || '', notes: item.notes || '' } : emptyWish)
    setDrawer('wish')
  }
  function openReminderForm(item = null) {
    setEditingReminder(item)
    setEditingWish(null)
    setReminderForm(item ? { title: item.title, dueDate: item.dueDate?.slice(0, 10) || '', amount: item.amount || '', notes: item.notes || '' } : emptyReminder)
    setDrawer('reminder')
  }
  ...
  async function submitWish(event) {
    ...
    try {
      const payload = { name: wishForm.name, amount, url: wishForm.url || null, notes: wishForm.notes || null }
      if (editingWish) {
        const updated = await wishService.updateWish(editingWish.id, payload)
        setWishes((items) => items.map((item) => item.id === updated.id ? updated : item))
        toast.success('Desejo atualizado.')
      } else {
        const created = await wishService.createWish(payload)
  ...
  async function submitReminder(event) {
    ...
    setIsSubmitting(true); setError('')
    try {
      const payload = { title: reminderForm.title, dueDate: reminderForm.dueDate || null, amount, notes: reminderForm.notes || null }
      if (editingReminder) {
        const updated = await wishService.updateReminder(editingReminder.id, payload)
        setReminders((items) => items.map((item) => item.id === updated.id ? updated : item))
        toast.success('Lembrete atualizado.')
      } else {
        const created = await wishService.createReminder(payload)
