...
const initialMovement = { type: 'EXPENSE', accountId: '', categoryId: '', subcategoryId: '', creditCardId: '', description: '', amount: '', date: today, dueDate: '', status: 'COMPLETED', paymentMethod: 'PIX', notes: '' }
...
export function TransactionsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [accounts, cards, categories, transactionResult, transfers] = await Promise.all([
        accountService.list('active'), cardService.list('active'), categoryService.list('active'), transactionService.list({ limit: 50 }), transferService.list(),
      ])
  ...
  function changeMovement(event) {
    ...
    setMovement((current) => ({ ...current, [name]: value, ...(name === 'type' ? { categoryId: '', subcategoryId: '' } : {}), ...(name === 'categoryId' ? { subcategoryId: '' } : {}), ...(name === 'paymentMethod' && value !== 'CREDIT_CARD' ? { creditCardId: '' } : {}), ...(name === 'paymentMethod' && value === 'CREDIT_CARD' ? { status: 'COMPLETED' } : {}) }))
  }
  ...
  async function submitMovement(event) {
    ...
    try {
      ...
      await transactionService.create({ ...movement, amount: parseCurrency(movement.amount), subcategoryId: movement.subcategoryId || null, dueDate: movement.dueDate || null, paymentMethod: movement.paymentMethod || null, notes: movement.notes || null })
  ...
  async function submitTransfer(event) {
    event.preventDefault(); setIsSubmitting(true); setError('')
    try {
      await transferService.create({ ...transfer, amount: parseCurrency(transfer.amount), description: transfer.description || null, idempotencyKey: crypto.randomUUID() })
  ...
  async function cancel(item) { if (!window.confirm(`Cancelar o lançamento "${item.description}"? O histórico será mantido e o saldo será revertido quando necessário.`)) return; try { await transactionService.cancel(item.id); await load() } catch (requestError) { setError(getApiError(requestError)) } }
  async function remove(item) { if (!window.confirm(`Excluir lançamento? Esta ação removerá permanentemente "${item.description}" e poderá alterar saldos e relatórios.`)) return; try { await transactionService.remove(item.id); await load() } catch (requestError) { setError(getApiError(requestError)) } }
  ...
  return <section className="page-stack">
    ...
    {mode === 'movement' ? <section className="editor-card"><form className="entity-form" onSubmit={submitMovement}>
      ...
      <label className="form-field"><span>Categoria</span><CategorySelect value={movement.categoryId} options={usableCategories} onChange={changeMovement} /></label>
      ...
      <label className="form-field"><span>Valor</span><CurrencyInput name="amount" value={transfer.amount} onChange={changeTransfer} required /></label>
