...
export function InvestmentsPage() {
  ...
  return <section className="page-stack">
    ...
    <div className="movement-list">{data.investments.map((investment) => <article className="movement-row movement-row--plain" key={investment.id}><div className="movement-info"><strong>{investment.name}</strong><small>{investment.type} · rentabilidade manual</small></div><div className="movement-value"><strong>{formatCurrency(investment.currentAmount, user.currency)}</strong><small className={Number(investment.profit) >= 0 ? 'income-text' : 'expense-text'}>{investment.profitPercent}%</small></div></article>)}</div>
