...
export function AccountsPage() {
  ...
  async function submit(event) {
    ...
    try {
      if (editingAccount) {
        ...
        if (balanceChanged)
          await accountService.adjustBalance(editingAccount.id, nextBalance);
      } else {
  ...
  async function submitBalance(event) {
    ...
    if (Number(nextBalance) === Number(balanceAccount.currentBalance)) {
      closeBalanceEditor();
      return;
    }
    if (
  ...
  const totalBalance = activeAccounts.reduce(
    (sum, account) =>
      sum + Number(account.projectedBalance ?? account.currentBalance),
    0,
  );
    ...
    <section className="page-stack">
      <div className="page-heading with-action">
        <div>
          ...
          <h1>Suas contas</h1>
          <p>
            Cadastre bancos, carteiras e dinheiro físico para formar seu saldo
            disponível.
          </p>
        </div>
        <button
          ...
          <div className="editor-heading">
            <div>
              ...
              <h2 id="account-form-title">
                {editingAccount ? editingAccount.name : "Adicionar uma conta"}
              </h2>
            </div>
          ...
          <form className="entity-form" onSubmit={submit}>
            ...
            {editingAccount ? (
              <label className="form-field">
                ...
                />
                <small>
                  O saldo inicial histórico será preservado. O ajuste fica
                  registrado no histórico da conta.
                </small>
              </label>
            ) : (
              <label className="form-field">
            ...
            <label className="form-field">
              <span>Ícone ou apelido visual</span>
              <input
                name="icon"
                value={form.icon}
                onChange={updateForm}
                maxLength="60"
                placeholder="Ex.: 💳 ou carteira"
              />
            </label>
            <button
        ...
        <section className="editor-card" aria-labelledby="balance-form-title">
          <div className="editor-heading">
            <div>
              ...
              <h2 id="balance-form-title">
                Editar saldo de {balanceAccount.name}
              </h2>
            </div>
            <button
          ...
          <form className="entity-form" onSubmit={submitBalance}>
            ...
            <p className="form-help form-field-wide">
              O ajuste Ã© registrado no histÃ³rico da conta e nÃ£o cria uma
              receita ou despesa.
            </p>
            <button
              className="primary-button"
              ...
              disabled={isSubmitting}
            >
              {isSubmitting ? "Salvando..." : "Confirmar ajuste de saldo"}
            </button>
          </form>
        </section>
      ) : null}
      ...
      {isLoading ? (
        <p className="loading-inline">Carregando contas...</p>
      ) : null}
      {!isLoading && accounts.length === 0 ? (
        <EmptyState
          title="Nenhuma conta cadastrada"
          description="Adicione sua primeira conta para começar a visualizar seu saldo."
          action={
            <button
              className="primary-button inline-button"
              type="button"
      ...
      {!isLoading && accounts.length > 0 ? (
        <div className="entity-grid">
          {accounts.map((account) => (
            <article
              className={`account-card ${account.isActive ? "" : "is-inactive"}`}
              key={account.id}
            >
              <div className="account-card-top">
                <span
                  className="color-chip"
                  style={{ backgroundColor: account.color || "#667085" }}
                  aria-hidden="true"
                >
                  {account.icon || account.name.slice(0, 1).toUpperCase()}
                </span>
                <div>
                  <h2>{account.name}</h2>
                  <p>
                ...
                {!account.isActive ? (
                  <span className="status-tag">Inativa</span>
                ) : null}
              </div>
              <strong>
                {formatCurrency(account.currentBalance, user.currency)}
              </strong>
              <small
                className={
                  Number(account.projectedBalance ?? account.currentBalance) < 0
                    ? "expense-text"
                    : ""
                }
              >
                ...
                {formatCurrency(
                  account.projectedBalance ?? account.currentBalance,
                  user.currency,
                )}
              </small>
              {Number(account.pendingCommitments) > 0 ? (
                <small>
                  Compromissos pendentes:{" "}
                  {formatCurrency(account.pendingCommitments, user.currency)}
                </small>
              ) : (
                <small>
                  Saldo inicial:{" "}
                  {formatCurrency(account.initialBalance, user.currency)}
                </small>
              )}
              <div className="card-actions">
                ...
                <button
                  type="button"
                  onClick={() => openBalanceEditor(account)}
                >
                  Editar saldo
                </button>
                <button type="button" onClick={() => toggleStatus(account)}>
