...
describe.sequential("despesa com cartao pela tela de lancamentos", () => {
  ...
  it("vincula compra, fatura e limite sem debitar a conta", async () => {
    const registration = await agent
      ...
      .send({
        name: "Fluxo cartao",
        email: `cartao-movimento-${randomUUID()}@example.test`,
    ...
    const categories = await agent.get(
      "/api/categories?type=EXPENSE&status=active",
    );
    categoryId = categories.body.categories.find(
    ...
    const missingCard = await agent
      .post("/api/transactions")
      .send({
        accountId,
        categoryId,
        type: "EXPENSE",
        description: "Sem cartao",
        amount: "10",
        date: "2026-08-11",
    ...
    const details = await agent.get(
      `/api/transactions/${created.body.transaction.id}`,
    );
    expect(details.status).toBe(200);
    expect(details.body.transaction).toMatchObject({
      creditCard: { id: cardId },
      cardPurchase: { installmentsCount: 1 },
    });
  ...
  it("gera parcelas pelo lançamento e considera somente a competência da fatura no dashboard", async () => {
    ...
    const invoices = await agent.get(`/api/cards/${cardId}/invoices`);
    const installments = invoices.body.invoices
      .flatMap((invoice) => invoice.installments)
      .filter((item) => item.purchase.description === "Compra parcelada")
    ...
    expect(
      installments.reduce((total, item) => total + Number(item.amount), 0),
    ).toBe(900);
