...
export async function getDashboard(userId, month) {
  const range = monthBounds(month); const completed = { userId, status: "COMPLETED", cardPurchaseId: null, creditCardInvoiceId: null, date: { gte: range.start, lt: range.end } };
  const [accounts, transactions, installments, pending, overdue, cards, upcomingInvoices] = await Promise.all([
    ...
    prisma.cardInstallment.findMany({ where: { userId, status: { not: "CANCELLED" }, invoice: { referenceYear: range.year, referenceMonth: range.month } }, include: { purchase: { include: { category: { select: { name: true } } } } } }),
  ...
  for (const item of installments) { expense = expense.plus(item.amount); add(categories, item.purchase.category.name, item.amount); }
